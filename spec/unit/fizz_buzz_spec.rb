describe FizzBuzz do
  ...
  describe '#display' do
    it 'prints fizz or buzz for a given numbers' do
      allow(fizz_buzz.fizz_buzzer).to receive(:fizz_or_buzz).with(3).and_return('Fizz')
      fizz_buzz.display(3)
      expect(test_output.get_writes).to eq %w(Fizz)
    end
  end
  ...
  describe '#display_each' do
    it 'prints fizz or buzz for a set of given numbers' do
      ...
      fizz_buzz.display_each([3, 5, 15, 7])
  ...
  describe '#initialize' do
    context "optional display" do
      ...
      it 'writes to the display' do
        expect{fizz_buzz.display(3)}.to change{output.get_writes.count}.from(0).to(1)
      end  
